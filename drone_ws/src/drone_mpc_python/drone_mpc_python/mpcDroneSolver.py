...
class DroneMPCSolver:
    ...
    #Constructor
    def __init__(self,accel_max=10,N_horizon=40,prediction_period=0.8,Q=np.diag([10.0,10.0,10.0,1.0,1.0,1.0]),R=np.diag([1.0,1.0,1.0])):

        self.Q = Q if not None else np.diag([10.0,10.0,10.0,1.0,1.0,1.0])
        self.R = R if not None else np.diag([1.0,1.0,1.0])
